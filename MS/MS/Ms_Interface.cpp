...
extern Ms_Logic logic;                // 게임 로직 객체 (다른 파일에서 정의됨)
extern int timerCount;                // 타이머 값 (초 단위)
 ...
 * @param hdc 현재 HDC
 * @param isRevealed 셀 클릭 여부 (true: 클릭됨, false: 닫힘)
 * @param x 셀의 X 좌표 (픽셀 단위)
 * @param y 셀의 Y 좌표 (픽셀 단위)
 */
void DrawCell(HDC hdc, bool isRevealed, int x, int y) {
    HBITMAP hBitmap = isRevealed ? hCellClickedBitmap : hCellBitmap; // 상태에 따른 비트맵 선택
    if (!hBitmap) return; // 비트맵이 로드되지 않은 경우 반환
...
/**
 * @brief 주변 지뢰 수를 나타내는 숫자 비트맵을 셀에 렌더링합니다.
 * @param hdc 현재 HDC
...
void DrawNumberBitmap(HDC hdc, int number, int x, int y) {
    if (number < 1 || number > 8) return; // 유효한 숫자가 아니면 반환

    HDC memDC = CreateCompatibleDC(hdc);
    HBITMAP oldBitmap = (HBITMAP)SelectObject(memDC, hAdjBombBitmap);

    // 숫자에 해당하는 비트맵의 X 좌표 계산
    int srcX = (number - 1) * CELL_WIDTH;

    // 비트맵 복사
    BitBlt(hdc, x, y, CELL_WIDTH, CELL_HEIGHT, memDC, srcX, 0, SRCCOPY);
...
/**
 * @brief 게임 보드를 초기화하고 셀 상태에 따라 렌더링합니다.
 * @param hwnd 윈도우 핸들
 * @param level 현재 난이도 객체 (Ms_level)
...
void InitializeBoard(HDC hdc, Ms_level& level, Ms_Logic& logic) {
    ...
    int startY = 100;                     // 보드 시작 Y 좌표 (UI 아래)

    const auto& board = logic.GetBoard(); // 보드 상태 가져오기
...
void LoadResources(HINSTANCE hInstance) {
    ...
    // 비트맵 로드 실패 처리
    if (!hCellBitmap || !hCellClickedBitmap || !hFlagBitmap || !hMineBitmap || !hAdjBombBitmap) {
        MessageBox(NULL, L"Failed to load one or more bitmaps!", L"Error", MB_ICONERROR);
    }
}
...
void UpdateRemainingMines(HWND hWnd, int remainingMines) {
    ...
    HDC hdc = GetDC(hWnd);
    HDC memDC = CreateCompatibleDC(hdc);
    HBITMAP hBitmap = CreateCompatibleBitmap(hdc, 130, 30); // 백 버퍼
    HBITMAP oldBitmap = (HBITMAP)SelectObject(memDC, hBitmap);
